# QUY TẮC BẮT BUỘC CHO AI AGENT — DỰ ÁN TRA CỨU KẾ TOÁN KIỂU VIỆT

> ⚠️ ĐÂY LÀ HỆ THỐNG QUY TẮC BẮT BUỘC CỦA DỰ ÁN NÀY.
...
1. **CẤM LÀM NHỎ GIỌT / LÀM NỬA CHỪNG (L01):**
   - Hệ thống có **55 văn bản pháp luật** trong `public/data/decrees.json`.
   - Mọi nhiệm vụ (so sánh điểm mới, hiển thị, tải file, kiểm tra render) PHẢI THỰC HIỆN ĐỦ 100% CẢ 55 VĂN BẢN.
   - Tuyệt đối CẤM làm vài ba cái đại diện, làm mẫu, hoặc tự ý cắt bớt danh sách.

2. **CẤM BÁO CÁO "XONG" KHI CHƯA CHẠY TEST THỰC TẾ (L05):**
   - Phải tự tay chạy script / trình duyệt kiểm tra (headless browser) trên toàn bộ danh sách 55 văn bản.
